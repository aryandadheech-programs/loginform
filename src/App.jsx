...
        <p className="text-sm text-slate-400">Next updates regarding to this is comming soon THANKYOU</p>
        
        <button 
          onClick={() => window.location.reload()} 
          className="mt-6 bg-rose-600 hover:bg-rose-500 text-white font-semibold py-2 px-6 rounded-xl transition-all"
        >
          Log Out
        </button>
      </div>
...
      // Safe Check: Check testing for endpoint string or backend token response
...
      // Direct Action Based on Success Response
      if (isLoginAction) {
        setTimeout(() => {
          setIsLoggedInSuccessfully(true); 
        }, 1000); // 1 second dashboard opening delay
      } else {
        // Safe signup redirect to login view
        setTimeout(() => {
          setIsLogin(true);
          setServerMessage({ type: '', text: '' });
        }, 1500);
...
            type="submit"
            disabled={loading}
            className="w-full mt-2 bg-indigo-600 hover:bg-indigo-500 text-white font-semibold py-3 rounded-xl disabled:opacity-50"
          >
